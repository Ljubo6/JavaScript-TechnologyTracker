...
function openCard(event) {
    ...
    openModel(toModal(tech),tech.title)
}

function toModal(tech){
    let checked = tech.done ? 'checked' : ''
    return `
    <h2>${tech.title}</h2>
...
function openModel(html,title = APP_TITLE){
...
function renderCards(){
    if (technologies.length === 0) {
        content.innerHTML = `<p class="empty"> There is not technology yet.Add it first, please!</p>`
    } else {
        let html = ''
